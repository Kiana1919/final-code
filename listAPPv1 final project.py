...
"""
Below are what we call if/elif/else statements they are used to define logic,
its also one of the easiest things you can use in coding.
We are using them to make a decision based if it is true or false.
You'll probbly notice that all of the code below is kinda like a big chunk of code,
thats because the if/elif/else statements normally have to be on the same indentation level.
"""
                if choice == "1":
                addToList()
            elif choice == "2":
                ...
                binSearch = input("What number are you looking for?")
                result = iterativeBinarySearch(unique_list, int(inSearch))
                if result != -1:
...
"""
Right below this multilined comment the code starts with the word def,
this key word is what's used to create funtions
"""
def addToList():
...
"""
indexValues()
Funtion explanation: we created a variable called 'indexPos', and stored
the result of an input function inside it.

We then force the value stored in indexPos into an integer (unsing the int() function)
